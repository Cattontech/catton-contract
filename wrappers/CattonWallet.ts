...
export type CattonWalletConfig = {};

export function CattonWalletConfigToCell(config: CattonWalletConfig): Cell {
    return beginCell().endCell();
}

export class CattonWallet implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) { }

    static createFromAddress(address: Address) {
        return new CattonWallet(address);
    }

    static createFromConfig(config: CattonWalletConfig, code: Cell, workchain = 0) {
        const data = CattonWalletConfigToCell(config);
        const init = { code, data };
        return new CattonWallet(contractAddress(workchain, init), init);
    }
    ...
    async sendTransfer(provider: ContractProvider, via: Sender,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: CattonWallet.transferMessage(jetton_amount, to, responseAddress, customPayload, forward_ton_amount, forwardPayload),
            value: value
        });
    ...
    */
    ...
    async sendBurn(provider: ContractProvider, via: Sender, value: bigint,
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: CattonWallet.burnMessage(jetton_amount, responseAddress, customPayload),
            value: value
        });
    ...
    */
    ...
    async sendWithdrawTons(provider: ContractProvider, via: Sender) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: CattonWallet.withdrawTonsMessage(),
            value: toNano('0.1')
        });
    ...
    */
    ...
    async sendWithdrawJettons(provider: ContractProvider, via: Sender, from: Address, amount: bigint) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: CattonWallet.withdrawJettonsMessage(from, amount),
            value: toNano('0.1')
        });
